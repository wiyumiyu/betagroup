-- OJO DESDE AQU� SE EMPIEZA A EJECUTAR DESDE EL ESQUEMA ADMIN_SYS
...
CREATE OR REPLACE PROCEDURE VALIDAR_LOGIN (
    ...
    p_pass IN VARCHAR2,           -- contrase�a que ingresa el usuario
    ...
    v_hash VARCHAR2(64);          -- variable para almacenar el hash de la contrase�a
BEGIN
    -- Hasheamos la contrase�a ingresada para compararla con la guardada
    v_hash := HASH_PASSWORD(p_pass);

    -- Buscamos un usuario que tenga ese correo y contrase�a
    SELECT id_usuario, nombre_usuario, rol
    INTO p_id_usuario, p_nombre, p_rol
    FROM USUARIO
    WHERE correo = p_correo AND contrasena = v_hash;

    -- Si encontr� el usuario, el login es v�lido
    p_resultado := 1;

EXCEPTION
    WHEN NO_DATA_FOUND THEN
        -- Si no se encontr�, el login es inv�lido
        p_resultado := 0;
END;
/
...
-- Si necesitas borrar todos los usuarios para hacer pruebas de nuevo, puedes usar esta l�nea:
-- DELETE FROM USUARIO;
...
INSERT INTO categoria ( nombre_categoria ) VALUES ( 'Equipos Est�ticos' );
...
INSERT INTO producto (
    ...
    'Soprano Titanium (Depilaci�n L�ser)',
    42000000,
    1,
    1
);
...
INSERT INTO producto (
    ...
    'Multifuncional 6 en 1 con Lipol�ser 850mz y EMS',
    800000,
    2,
    1
);
