...
BEGIN
    ...
    -- Eliminar los tel�fonos asociados
...
--46. Procedimiento para obtener tel�fonos de un cliente
...
--47. Procedimiento para obtener solo los IDs de tel�fono de un cliente
...
BEGIN
  IF INSERTING THEN
    v_operacion := 'INSERT';
    v_descripcion := 'Se insert� el producto: ' ||
                     'ID=' || :NEW.ID_PRODUCTO || ', ' ||
  ...
  ELSIF UPDATING THEN
    v_operacion := 'UPDATE';
    v_descripcion := 'Se actualiz� el producto ID=' || :OLD.ID_PRODUCTO || ' ? ' ||
                     'ANTES [NOMBRE=' || :OLD.NOMBRE_PRODUCTO || ', PRECIO=' || :OLD.PRECIO || '] ? ' ||
                     'DESPU�S [NOMBRE=' || :NEW.NOMBRE_PRODUCTO || ', PRECIO=' || :NEW.PRECIO || ']';

  ELSIF DELETING THEN
    v_operacion := 'DELETE';
    v_descripcion := 'Se elimin� el producto: ' ||
                     'ID=' || :OLD.ID_PRODUCTO || ', ' ||
...
BEGIN
  ...
  ELSIF UPDATING THEN
    ...
    v_descripcion := 'Se actualiz� la venta ID=' || :OLD.ID_VENTA || ' ? ' ||
                     ...
                     'DESPU�S [NUMERO=' || :NEW.NUMERO || ', IMPUESTOS=' || :NEW.IMPUESTOS || ']';
...
BEGIN
  ...
  ELSIF UPDATING THEN
    ...
    v_descripcion := 'Se actualiz� el detalle de venta ID=' || :OLD.ID_VENTA_DETALLE || ' ? ' ||
                     ...
                     'DESPU�S [CANTIDAD=' || :NEW.CANTIDAD || ', PRECIO_UNITARIO=' || :NEW.PRECIO_UNITARIO || ', DESCUENTO=' || :NEW.DESCUENTO || ']';
