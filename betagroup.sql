...
-- 1. Funcion para encriptar la contraseï¿½a usando SHA-256
...
BEGIN
    -- Eliminar primero records de VENTA_DETALLE
    DELETE FROM VENTA_DETALLE
    WHERE ID_PRODUCTO IN (
        SELECT ID_PRODUCTO FROM PRODUCTO WHERE ID_PROVEEDOR = p_id_proveedor
    );
    ...
    -- Eliminar teléfonos asociados al proveedor
    DELETE FROM TELEFONO_PROVEEDOR
    WHERE ID_PROVEEDOR = p_id_proveedor;

    -- Por ultimo, eliminar el proveedor
